...
const epic5: Epic<FluxStandardAction, State> = (action$, store) =>
  action$.ofType('FIFTH').mergeMap(({ type, payload }) =>
    Observable.of({
      type: 'fifth',
      payload
    })
...
const epic6: Epic<FluxStandardAction, State> = (action$, store) =>
  action$.ofType('SIXTH').map(({ type, payload }) => ({
    type: 'sixth',
    payload
  }));

const epic7: Epic<FluxStandardAction, State, Dependencies> = (
  action$,
  store,
  dependencies
) =>
  action$.ofType('SEVENTH').map(({ type, payload }) => ({
    type: 'seventh',
    payload: dependencies.func(payload)
  }));

const epic8: Epic<FluxStandardAction, State, Dependencies> = (
  action$,
  store,
  dependencies
) =>
...
const epic9_1: Epic<FluxStandardAction, State, Dependencies, Epic9_Output> = (
  action$,
  store,
  dependencies
) =>
...
const epic9_2 = (
  action$: ActionsObservable<FluxStandardAction>,
...
const rootEpic1: Epic<FluxStandardAction, State> = combineEpics<
  FluxStandardAction,
  State
>(epic1, epic2, epic3, epic4, epic5, epic6, epic7, epic8, epic9_1);
const rootEpic2 = combineEpics(
  epic1,
  epic2,
  epic3,
  epic4,
...
const epicMiddleware1: EpicMiddleware<
  FluxStandardAction,
  State
> = createEpicMiddleware<FluxStandardAction, State>(rootEpic1, {
  dependencies
});
const epicMiddleware2 = createEpicMiddleware(rootEpic2, { dependencies });

interface CustomEpic<T extends Action, S, U> {
  (action$: ActionsObservable<T>, store: MiddlewareAPI<S>, api: U): Observable<
    T
  >;
}

const customEpic: CustomEpic<FluxStandardAction, State, number> = (
  action$,
  store,
  some
) =>
  action$.ofType('CUSTOM1').map(({ type, payload }) => ({
    type: 'custom1',
    payload
  }));

const customEpic2: CustomEpic<FluxStandardAction, State, number> = (
  action$,
  store,
  some
) =>
  action$.ofType('CUSTOM2').map(({ type, payload }) => ({
    type: 'custom2',
    payload
  }));

const customEpicMiddleware: EpicMiddleware<
  FluxStandardAction,
  State
> = createEpicMiddleware<FluxStandardAction, State>(rootEpic1, {
...
const combinedCustomEpics = combineEpics<
  CustomEpic<FluxStandardAction, State, number>
>(customEpic, customEpic2);

const reducer = (
  state: Array<FluxStandardAction> = [],
...
const action$1: ActionsObservable<FluxStandardAction> = new ActionsObservable<
  FluxStandardAction
>(input$);
const action$2: ActionsObservable<FluxStandardAction> = ActionsObservable.of<
  FluxStandardAction
>({ type: 'SECOND' }, { type: 'FIRST' }, asap);
const action$3: ActionsObservable<FluxStandardAction> = ActionsObservable.from<
  FluxStandardAction
>([{ type: 'SECOND' }, { type: 'FIRST' }], asap);
